...
async function startStream(title: string = 'Claude Code Session', isPrivate: boolean = false): Promise<string> {
  if (streamProcess) {
    ...
    switch (name) {
      case 'stream_start':
        result = await startStream(
          (args as any)?.title || 'Claude Code Session',
          (args as any)?.private || false
        );
        break;
      ...
      case 'stream_chat':
        result = await sendChat((args as any)?.message || '');
        break;
