...
// History/Archive operations
  ...
  async getDailyStreamingStats(days: number = 14): Promise<Array<{
    date: string;
    streamCount: number;
    uniqueAgents: number;
    totalViewers: number;
    ...
    const statsMap = new Map<string, { streamCount: number; uniqueAgents: number; totalViewers: number }>();
    ...
    const stats: Array<{ date: string; streamCount: number; uniqueAgents: number; totalViewers: number }> = [];
  ...
  async addChatReaction(
    messageId: string,
    roomId: string,
    userId: string,
    reaction: 'thumbs_up' | 'thumbs_down'
  ): Promise<{ success: boolean; thumbsUp: number; thumbsDown: number }> {
    const id = uuidv4();
    const now = Date.now();

    // Upsert reaction (update if exists, insert if not)
    await this.pool.query(
      `INSERT INTO chat_reactions (id, message_id, room_id, user_id, reaction, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (message_id, user_id) DO UPDATE SET reaction = $5`,
      [id, messageId, roomId, userId, reaction, now]
    );
  ...
  async getChatReactionCounts(messageId: string): Promise<{ thumbsUp: number; thumbsDown: number }> {
    const result = await this.pool.query(
      `SELECT
  ...
  async getUserReaction(messageId: string, userId: string): Promise<string | null> {
    const result = await this.pool.query(
      `SELECT reaction FROM chat_reactions WHERE message_id = $1 AND user_id = $2`,
      [messageId, userId]
    );
    return result.rows[0]?.reaction || null;
  }

  async getChatReactionsForMessages(messageIds: string[]): Promise<Map<string, { thumbsUp: number; thumbsDown: number }>> {
