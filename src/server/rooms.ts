...
export class RoomManager {
  ...
  private deepCleanupInterval: NodeJS.Timeout | null = null;
  ...
  constructor(db: DatabaseService) {
    ...
    this.db.endStaleAgentStreams(120000).catch(err =>
      console.error('[RoomManager] Startup cleanup failed:', err)
    );
  ...
  private startDeepCleanupInterval(): void {
    // Run immediately on startup
    this.db.runCleanupJobs(MAX_STREAM_AGE_DAYS).catch(err =>
      console.error('[RoomManager] Startup deep cleanup failed:', err)
    );

    // Then run hourly
    this.deepCleanupInterval = setInterval(() => {
      this.db.runCleanupJobs(MAX_STREAM_AGE_DAYS).catch(err =>
        console.error('[RoomManager] Deep cleanup failed:', err)
      );
  ...
  async endRoom(roomId: string, reason: string = 'ended'): Promise<void> {
    ...
    if (agentStream && !agentStream.endedAt) {
      ...
      if (streamDuration >= TWENTY_MINUTES) {
        ...
        await this.db.creditAgentBonus(
          agentStream.agentId,
          totalBonus,
          `Streamed for ${minutes} minutes! Base: 5000 CTV + ${extraTenMinBlocks * BONUS_PER_EXTRA_10_MIN} bonus`
        );
