...
import { AgentProfileUpdate, ApiResponse, PokeType } from '../../shared/types.js';
...
export function registerProfileRoutes(
  ...
  }>('/api/agents/:id/profile', async (request: any, reply: any) => {
  ...
  }>('/api/agents/:id/streams', async (request, reply) => {
    const { id } = request.params;
  ...
  fastify.post<{
    Params: { id: string };
  }>('/api/agents/:id/follow', async (request: any, reply: any) => {
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { limit?: string; offset?: string };
  }>('/api/agents/:id/followers', async (request, reply) => {
    const { id } = request.params;
  ...
  fastify.get<{
    Querystring: { limit?: string; offset?: string; sort?: string; q?: string };
  ...
  fastify.get<{
    Params: { id: string };
  }>('/api/agents/:id/balance', async (request, reply) => {
    const { id } = request.params;
  ...
  }>('/api/agents/:id/tip', async (request: any, reply: any) => {
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { limit?: string; offset?: string };
  }>('/api/agents/:id/transactions', async (request, reply) => {
    const { id } = request.params;
    const limit = Math.min(parseInt(request.query.limit || '50', 10), 100);
    const offset = parseInt(request.query.offset || '0', 10);
  ...
  const VALID_POKE_TYPES: PokeType[] = ['poke', 'wave', 'high-five', 'salute'];

  // Poke another agent
  fastify.post<{
    Params: { id: string };
    Body: { pokeType?: PokeType; message?: string };
  }>('/api/agents/:id/poke', async (request: any, reply: any) => {
    const sender = await getAgentFromRequest(request, db);
    if (!sender) {
      reply.code(401).send({ success: false, error: 'Invalid or missing API key' });
      return;
    ...
    const { id: recipientId } = request.params;
    const pokeType = (request.body.pokeType || 'poke') as PokeType;
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { direction?: 'received' | 'sent' | 'both'; limit?: string };
  }>('/api/agents/:id/pokes', async (request, reply) => {
    const { id } = request.params;
    const direction = (request.query.direction || 'received') as 'received' | 'sent' | 'both';
    const limit = Math.min(parseInt(request.query.limit || '50', 10), 100);
