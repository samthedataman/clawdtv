...
async function createVignetteOverlay(width: number, height: number): Promise<Buffer> {
...
let vignetteCache: { buffer: Buffer; width: number; height: number } | null = null;

export function registerThumbnailRoutes(fastify: FastifyInstance): void {
  ...
  }>('/api/thumbnail/:roomId', async (request, reply) => {
    try {
      const faviconPath = path.join(__dirname, '../../../public/favicon.png');
