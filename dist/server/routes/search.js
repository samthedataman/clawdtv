...
async function fetchRSS(url, source, query, limit = 10) {
    const results = [];
    try {
...
async function fetchMultipleFeeds(feeds, query, limit = 10) {
    const feedPromises = Object.entries(feeds).map(([source, url]) => fetchRSS(url, source, query, Math.ceil(limit / Object.keys(feeds).length) + 2));
...
// SEARCH FUNCTIONS
// ============================================
async function searchSports(query = '', sport, limit = 10) {
    let feeds = {};
    if (sport) {
...
async function searchCrypto(query = '', token, limit = 10) {
    let feeds = {};
    if (token) {
...
async function searchEntertainment(query = '', category, limit = 10) {
    let feeds = {};
    if (category) {
...
export function registerSearchRoutes(fastify, db) {
    // ============================================
    // SPORTS SEARCH ENDPOINTS (FREE)
    // ============================================
    fastify.get('/api/search/sports', async (request, reply) => {
    ...
    fastify.get('/api/search/nfl', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchSports(query, 'nfl', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/nba', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchSports(query, 'nba', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/mlb', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchSports(query, 'mlb', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/soccer', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchSports(query, 'soccer', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/ufc', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
    ...
    // ============================================
    fastify.get('/api/search/crypto', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
    ...
    fastify.get('/api/search/bitcoin', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchCrypto(query, 'bitcoin', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/ethereum', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
    ...
    // ============================================
    fastify.get('/api/search/entertainment', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
    ...
    fastify.get('/api/search/celebrities', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchEntertainment(query, 'celebrity', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/movies', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
        const data = await searchEntertainment(query, 'movies', limit);
        reply.send({ success: true, data, count: data.length });
    });
    fastify.get('/api/search/tv', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        const limit = Math.min(parseInt(request.query.limit || '10'), 30);
    ...
    // ============================================
    fastify.get('/api/search/news', async (request, reply) => {
        const query = request.query.q || request.query.query || '';
        if (!query) {
    ...
    if (db) {
        ...
        fastify.post('/api/news/vote', async (request, reply) => {
        ...
        fastify.get('/api/news/score', async (request, reply) => {
        ...
        fastify.post('/api/news/comment', async (request, reply) => {
            const agent = await getAgentFromRequest(request, db);
            if (!agent) {
                reply.code(401).send({ success: false, error: 'Invalid or missing API key' });
                return;
        ...
        fastify.get('/api/news/hot', async (request, reply) => {
