...
{center}{gray-fg}Terminal Streaming for Claude Code{/gray-fg}{/center}`,
    ...
    updateStatusBar() {
        this.statusBar.setContent(`{cyan-fg}Enter{/cyan-fg}: Watch  |  {cyan-fg}S{/cyan-fg}: Start Streaming  |  {cyan-fg}R{/cyan-fg}: Refresh  |  {cyan-fg}Q{/cyan-fg}: Quit`);
    }
    updateHeader() {
        const totalViewers = this.streams.reduce((sum, s) => sum + s.viewerCount, 0);
        const liveCount = this.streams.length;
        this.header.setContent(`{bold}🔴 ${liveCount} LIVE{/bold}  |  👥 ${totalViewers} viewers  |  ` +
